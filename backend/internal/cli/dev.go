...
func newDevImportProjectsCommand(ctx *commandContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "import-projects",
		Short: "Copy project registry data into the current Square data dir",
		Long: "Copy active project registry rows from the normal Square data dir into " +
			"the current SQUARE_DATA_DIR. This copies only project metadata, project config, " +
	...
	cmd.Flags().StringVar(&opts.fromDataDir, "from-data-dir", "", "Square data dir to read (default ~/.square/data)")
...
func defaultNormalDataDir() (string, error) {
	...
	return filepath.Join(home, ".square", "data"), nil
}
