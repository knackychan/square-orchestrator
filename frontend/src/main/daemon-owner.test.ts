...
describe("keepDaemonAlive", () => {
	...
	it("returns false when AO_KEEP_DAEMON is empty", () => {
		expect(keepDaemonAlive({ SQUARE_KEEP_DAEMON: "" })).toBe(false);
	});

	it.each(["1", "true", "TRUE", "yes", "on", "ON", "Yes"])("returns true for truthy value %j", (value) => {
		expect(keepDaemonAlive({ SQUARE_KEEP_DAEMON: value })).toBe(true);
	});
	...
	it.each(["0", "false", "FALSE", "off", "OFF", "no", "No"])("returns false for conventional off value %j", (value) => {
		expect(keepDaemonAlive({ SQUARE_KEEP_DAEMON: value })).toBe(false);
	});
	...
	it("trims surrounding whitespace before evaluating", () => {
		expect(keepDaemonAlive({ SQUARE_KEEP_DAEMON: "  0  " })).toBe(false);
		expect(keepDaemonAlive({ SQUARE_KEEP_DAEMON: "  1  " })).toBe(true);
	});
});
