...
- Naming: every milestone is `SAxx — Short title`, every task `SAxx-Tyy — Short name`, every gate `Ax — Gate name`; the canonical registry is `plans/TASK_INDEX.md`. Use the readable name in prose, commits, and receipts — the bare code is only for machine fields and filenames.
...
2. Run SA00-T01 — Create and pin the downstream fork using its full task packet.
3. Run SA00-T02 — Capture the unchanged Windows build/test/package baseline and preserve it.
4. Run SA00-T03 — License/attribution/identity/telemetry/updater/data isolation design,
   SA00-T04 — Import authority and activate architecture amendment, then gate
   SA00-T05 — Adoption gate A0.
5. Complete SA01 — Windows lifecycle and AO platform hardening and gate A1 before relying on it.
6. Complete SA02 — Session-first contracts in order and freeze `square.contracts 1.0-draft`.
7. Complete SA03 — Durable state, SA04 — Execution facade, and SA05 — Session API.
8. Build SA06 — Session-first UI from fixtures in parallel only after A2; integrate after A5.
9. Complete SA07 — QUICK vertical slice and use Square Core Alpha internally before PLANNED.
10. Complete SA08 — Interactions and SA09 — Memory/context.
11. Complete SA10 — PLANNED and SA11 — MVP gate.
12. Add route plurality/clients/resources/release only after MVP evidence.
13. Keep SA15 — Optional advanced scale disabled.
```
...
Use `plans/KICKOFF_PROMPT_SA00-T01.md` and `plans/tasks/SA00-T01.md` for **SA00-T01 — Create and pin the downstream fork**. Do not begin with rebranding, UI implementation, persistence migrations, or Square workflow code.
