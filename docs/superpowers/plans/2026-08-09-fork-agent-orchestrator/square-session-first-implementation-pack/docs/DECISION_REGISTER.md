...
|---|---|
| owner Git hosting/origin URL | SA00-T01 — Create and pin the downstream fork (input) |
| final Windows app ID, publisher, installer GUIDs, signing identity | SA14-T04 — Square Windows identity, installer, data migration, and coexistence |
| Square update feed and channel policy | SA14-T05 — Square updater, live-work gate, backup, rollback |
| loopback API install token/authentication | SA14-T02 — Local API and Electron bridge hardening |
| exact first real adapter/model route | SA07-T07 — First real certified route (Alpha gate A7) after SA04 certification |
| default role routes/models | SA12-T01 — Global/project/session/task role-profile resolution |
| exact model names in UI | runtime route registry, never hard-coded as authority |
| project/global memory retention defaults | SA09 — Project/global memory and bounded context |
| terminal-history retention | SA03-T05 — Content-addressed artifacts / SA13-T05 — Buffered persistence |
| VS Code inclusion in MVP vs post-MVP | SA12-T05 — Minimal VS Code client (owner review) |
| advanced docking library | only when basic session docks prove insufficient |
| cost/exposure formulas | SA13 — Exposure, cost, resource profiles, context reuse, and evaluation |
| resource thresholds | SA13 measured evidence |
| parallel writers | SA15 — Optional advanced scale (disabled by default) |
